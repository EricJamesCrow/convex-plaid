...
// =============================================================================
// CONVEX DATA TRANSFORM FUNCTIONS
// These transform data from our Convex component's schema format
// =============================================================================

/**
 * Convert milliunits to currency string
 * Our component stores amounts as milliunits (amount × 1000) to avoid float issues
 */
const formatMilliunits = (milliunits: number | null | undefined): string => {
  if (milliunits == null) return "no data";
  const dollars = milliunits / 1000;
  return ` ${dollars.toFixed(2).toLocaleString()}`;
};

// Convex transaction schema
...
interface ConvexAccount {
  ...
  mask?: string;
  balances: {
    available?: number; // milliunits
    current?: number; // milliunits
    limit?: number; // milliunits
  };
}

// Convex liability schema
interface ConvexLiability {
  accountId: string;
  isOverdue: boolean;
  minimumPaymentAmount?: number; // milliunits
  nextPaymentDueDate?: string;
  lastStatementBalance?: number; // milliunits
  lastPaymentAmount?: number; // milliunits
...
export const transformConvexTransactionsData = (
...
export const transformConvexBalanceData = (
  accounts: ConvexAccount[]
): Array<DataItem> => {
  return accounts.map((account) => ({
    name: account.name,
    current: formatMilliunits(account.balances?.current),
    available: formatMilliunits(account.balances?.available),
    subtype: account.subtype || null,
    mask: account.mask || "",
  }));
};

export const transformConvexLiabilitiesData = (
  liabilities: ConvexLiability[]
): Array<DataItem> => {
  return liabilities.map((liability) => ({
    name: `Account ${liability.accountId.slice(-4)}`,
    type: "credit card",
    date: liability.lastPaymentDate || liability.nextPaymentDueDate || "N/A",
    amount: formatMilliunits(liability.lastPaymentAmount || liability.minimumPaymentAmount),
  }));
};
